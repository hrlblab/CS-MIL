    ...
    def _create_bags(self, index):
        ...
        if cluster_pickup.sum() == 0:
            ...
            for cl in range(len(cluster_pickup)):
                ...
                if pickup_num == 0:
                    features = np.zeros((1, 2048, 1, 1), dtype=np.float32)
                    mask[cnt] = 0
                    cnt = cnt + 1
                    labels = [now_label]
                else:
                    ...
                    features = np.zeros((pickup_num, 2048, 1, 1))

                    for fi in range(pickup_num):
                        features[fi] = np.expand_dims(np.expand_dims(np.load(features_root[fi]), 1), 1)
                        cnt = cnt + 1
