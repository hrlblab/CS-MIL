...
class MILBags(data_utils.Dataset):
    def __init__(self, data_csv_root, feature_csv_root, cluster_num, cluster_start, cluster_list, size = (256, 256), mean_bag_length = 64, var_bag_length = 4, seed=1, train=True):
        ...
        def count_cluser(df, cluster_num, cluster_start):
            list = []
            cluster_cnt = np.zeros((cluster_num))
            for ci in range(cluster_start, cluster_start + cluster_num):
                cluster_df = df[df['cluster'] == ci]
                cluster_cnt[ci - cluster_start] = len(cluster_df)
        ...
        for ki in range(len(self.data_csv)):
            ...
            if (self.train == 1) and (train == 1):
                ...
                now_cluster_df, now_cluster_cnt = count_cluser(now_df, self.cluster_num, self.cluster_start)
    ...
    def _create_bags(self, index):
        non_zero_idx_list = np.nonzero(self.clustercnt_list[index] > 0)[0]
        if len(non_zero_idx_list) == 0:
            return self.clustercnt_list, self.clustercnt_list, self.clustercnt_list, self.clustercnt_list, self.clustercnt_list, 1
        else:
        ...
        for ci in range(len(cluster_pickup)):
            if not ci + self.cluster_start in self.cluster_target_list:
                cluster_pickup[ci] = 0
            else:
        ...
        if cluster_pickup.sum() == 0:
            return cluster_pickup, cluster_pickup, cluster_pickup, cluster_pickup,cluster_pickup, 1
            ...
            for cl in range(len(cluster_pickup)):
                ...
                if pickup_num == 0:
                    ...
                    for fi in range(pickup_num):
                        ...
                        features[fi, :, 1] = np.expand_dims(
                            np.load(features_root[fi].replace('size256', 'size512')), 1)
                        features[fi,:, 2] = np.expand_dims(
                            np.load(features_root[fi].replace('size256', 'size1024')), 1)
                        cnt = cnt + 1

                    labels = [now_label] * pickup_num

                for ki in range(len(features_root)):
                    if features_root != 'aaa':
                        features_root[ki] = features_root[ki]#.replace('size256', 'size1024')

                cluster_bag.append((features))
                cluster_label.append((labels))
                cluster_features_root.append((features_root))

        return cluster_bag, [now_label, cluster_label], mask, case_name, cluster_features_root, 0
    ...
    def __getitem__(self, index):
        ...
        for ki in range(100):
            bag, label, mask, case_name, features_root, skip = self._create_bags(index)
            if skip == 1:
                continue
            ...
            features_root_list.append((features_root))

        if len(bag_list) == 0:
            return bag_list, label_list, mask_list, case_name, features_root_list, 1
        else:
            return bag_list, label_list, mask_list, case_name, features_root_list, 0
