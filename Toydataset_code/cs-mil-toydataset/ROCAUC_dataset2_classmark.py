...
attention_df = pd.read_csv('results_stage1_clustering_dataset2/attention_score_dataset2_region.csv')

region_image_folder = '/Data3/GCA_Demo/4864patch_dataset2'
region_folder = '/Data3/GCA_Demo/4864patch_dataset2_patch'
attention_folder = '/Data3/GCA_Demo/4864patch_dataset2_attention'
...
if get_bagscore_flag:
    ...
    for bi in range(int(len(attention_df)/8)):
        ...
        for ki in range(8):
            ...
            patch_red = img[now_x - 384* 2:now_x + patch_size - 384* 2, now_y - 384* 2:now_y + patch_size - 384* 2, 0].mean()
            patch_green = img[now_x - 384* 2:now_x + patch_size - 384* 2, now_y - 384* 2:now_y + patch_size - 384* 2, 1].mean()
    ...
    bag_score_df.to_csv('results_stage1_clustering_dataset2/bag_score.csv', index = False)

else:
    bag_score_df = pd.read_csv('results_stage1_clustering_dataset2/bag_score.csv')
...
plt.savefig('ROC-AUC_dataset2.png')
plt.clf()
...
plt.legend(loc='lower right')
plt.savefig('PR-AP_dataset2.png')
plt.clf()
