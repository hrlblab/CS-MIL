...
class MILBags(data_utils.Dataset):
    ...
    def _create_bags(self, index):
        ...
        if cluster_pickup.sum() == 0:
            ...
            for cl in range(len(cluster_pickup)):
                ...
                if pickup_num == 0:
                    features = np.zeros((1, 2048, 3), dtype=np.float32)
                    mask[cnt] = 0
                    cnt = cnt + 1
                    labels = [now_label]
                else:
                    ...
                    features = np.zeros((pickup_num, 2048, 3))

                    for fi in range(pickup_num):
                        '256'
                        features[fi,:,0] = np.load(features_root[fi])
                        features[fi,:,1] = np.load(features_root[fi].replace('size256', 'size512'))
                        features[fi,:,2] = np.load(features_root[fi].replace('size256', 'size1024'))
                        cnt = cnt + 1
    ...
    def __getitem__(self, index):
        ...
        return bag, label, mask, case_name, skip
...
if __name__ == "__main__":
    ...
    train_loader = data_utils.DataLoader(MILBags(data_csv_root = '/Data2/GCA/AttentionDeepMIL-Resnet50/data_split/data_list_0.csv',
                                                 feature_csv_root = '/Data2/GCA/simsiam/feature_cluster_csv_global',
                                                 cluster_num = 8,
                                                 cluster_list = [2,4,5,7],
                                                   size = (256, 256),
                                                   mean_bag_length=64,
                                                   var_bag_length=4,
                                                   seed=1,
    ...
    # len_bag_list_train = []
    # mnist_bags_train = 0
    # for batch_idx, (bag, label, mask, _, skip) in enumerate(train_loader):
    ...
    # len_bag_list_test = []
    # mnist_bags_test = 0
    # for batch_idx, (bag, label, mask, _, skip) in enumerate(Testing_loader):
