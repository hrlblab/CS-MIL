    ...
    def _create_bags(self, index):
        ...
        for ii in range(len(df)):
            if self.MIL:
                ...
                images_bags[ii, 0, ...] = plt.imread(now_images[ii].replace('size1024', 'size1024'))[:, :, :3]
                images_bags[ii, 1, ...] = plt.imread(now_images[ii].replace('size1024', 'size1024'))[:, :, :3]
                images_bags[ii, 2, ...] = plt.imread(now_images[ii].replace('size1024', 'size1024'))[:, :, :3]
...
if __name__ == "__main__":
    ...
    validation_loader = data_utils.DataLoader(MILBags(bag_root = '/Data2/GCA_Demo/Bag_1000_1/Val',
                                                  MIL = False),
                                        batch_size=1,
                                        shuffle=False)
